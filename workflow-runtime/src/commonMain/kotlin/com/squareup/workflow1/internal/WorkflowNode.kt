...
internal class WorkflowNode<PropsT, StateT, OutputT, RenderingT>(
  ...
  fun Rendering(
    ...
    RenderingWithStateType(wf, input, rendering)
  }
  ...
  private fun RenderingWithStateType(
    ...
    val (baseRenderContext, renderContext) = remember(state.value, workflow) {
      // Each state only uses the RenderContext once. After rendering it is frozen and we
      // must wait for a new state.
